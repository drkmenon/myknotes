...
Our immediate hunch will be to distribute the asset in all possible values of x to get an expected pay-off which is greater than the sum of the individual returns. Problem with this approach is the fact that we are neglecting the play of probability where the moral expectation is the true pay-off or return. In other words, the true return is not the arithmetic mean, it is the geometric mean of probability weighted return on capital.

## Best of both worlds

What we need is not indiscriminate diversification. We can domesticate the uncertainty basically by two methods:
